...
export default class TelegramApi {
	...
	private async fetchAndLog(url: Request, slug: string, data: TelegramApiParams): Promise<Response> {
		...
		if (response.status !== 200) {
			...
			throw new Error(
				`Telegram API error: ${String(response.status)} ${response.statusText}${errorDescription ? ': ' + errorDescription : ''}`,
			);
		}
		const cloned = response.clone();
		try {
	 ...
	 * @returns Promise with the API response
	 */
	async editMessageText(botApi: string, data: EditMessageTextParams): Promise<Response> {
		const url = this.getApiUrl(botApi, 'editMessageText', data, 'POST');
		return await this.fetchAndLog(url, 'editMessageText', data);
	}

	async sendMessageDraft(botApi: string, data: SendMessageDraftParams): Promise<Response> {
